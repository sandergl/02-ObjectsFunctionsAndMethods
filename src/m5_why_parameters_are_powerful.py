...
def main():
    ...
    #better_draw_circles(rg.Point(0, 0), 6)
    #better_draw_circles(rg.Point(-200, -200), 3)
    even_better_draw_circles(rg.Point(200, 200), 20, 4, 'red', 10)
    even_better_draw_circles(rg.Point(-200,-200), 10, 8, 'blue', 5)
...
def better_draw_circles(point, increase_size):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(increase_size)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(increase_size * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point, increase_size, times, color, thickness):
    turtle = rg.SimpleTurtle()
    ...
    for k in range(1, (int(times)+1)):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(increase_size * k)
